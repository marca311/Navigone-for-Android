...
public class MSSuggestions extends AsyncTask<String, Integer, MSLocation[]>
	...
	private MSLocation[] setLocationArray() {
		...
		for (int i = 0; i < locationNodeList.getLength(); i++) {
			...
			if (currentNode.getNodeType() == 1) {
				...
				MSLocation newLocation = MSSegment
						.setLocationClass(currentElement);
		...
		return locations;
		/*
		 * do { MSLocation test = MSSegment.setLocationClass(locationElement);
		 * locationsList.add(test); if (test == null) {
		 * System.out.println("IS NULL"); } else {
		 * System.out.println(test.getHumanReadable()); } } while
		 * ((locationElement = XMLParser.getElementSibling(locationElement)) !=
		 * null); for (int i = 0; i < locationsList.size(); i++) { locations[i]
		 * = locationsList.get(i); }
		 */
	}
	...
	public void onFocusChange(View v, boolean hasFocus) {
		/*
		 * if (hasFocus) { EditText textBox =
		 * (EditText)parentActivity.findViewById(v.getId()); LayoutParams
		 * boxParams = textBox.getLayoutParams(); ArrayAdapter<String> array =
